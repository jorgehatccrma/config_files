...
CLONE_SESSION="$(tput setaf 2)Clone an existing tmux Session$(tput sgr0)"
BASH_SESSION="$(tput setaf 2)Bash$(tput sgr0)"

# present menu for user to choose which workspace to open
PS3="Please choose your session: "
options=("$NEW_VANILLA_SESSION" "$NEW_DEV_SESSION" "$CLONE_SESSION" $(tmux list-sessions -F "#S" 2>/dev/null) "$BASH_SESSION")
...
do
  case $opt in
    ...
    $CLONE_SESSION)
      read -p "Enter target session name: " TARGET_SESSION
      CLONED_SESSION="${TARGET_SESSION}-clone"

      tmux new-session -d -t "$TARGET_SESSION" -s "$CLONED_SESSION"
      tmux -2 attach-session -d -t "$CLONED_SESSION"
